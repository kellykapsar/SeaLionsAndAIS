...
corrs <- rs_data_subset %>%
  select(all_of(covar_names)) %>% # Isolate just covars
  correlate() %>% # Calculate pairwise correlations
  shave() %>% # Remove redundant values
  stretch() %>% # Convert to long format
  filter(abs(r) > 0.6) %>% # Remove correlations <0.6
  mutate(x = getcovarcol(x, lookup), y = getcovarcol(y, lookup)) # Convert column names to matrix numbers
...
write.csv(master, paste0(resultdir, "ModelVariableList_", ind, ".csv"))
