...
rs.data.path <- paste(datadir, "/UsedAndAvail_WeeklyKDE_20211104.rds", sep = "")
...
j <- 1
  ...
  # calculate number of weeks of telemetry data for random effect
  ...
  data <- list(
    ...
    wk = rs_data_subset$weeklyhr_id[ind1], 
    x = x,
    y = rep(1, N),
    obs=c(1,0,0,0,0,0),
    pos = diag(1, 6)
  ...
  params <- c('mu', 'stdev', 'chis_obs', 'chis_sim')
  
  fit <- sampling(mod, data = data, pars = params, init = inits,
                  chains = 4=2, iter = 1000, warmup = 200, thin = 1)
  save(fit, "FIT.rds")
