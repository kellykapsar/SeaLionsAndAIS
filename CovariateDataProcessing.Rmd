...
# Crop to study area 
dist500m <- dist500m %>% raster::crop(study)

# Save results
# writeRaster(dist500m, "../Data_Processed/Dist500m_New.tif")


# Plot results
dist500m.df <- as.data.frame(dist500m, xy=TRUE) %>% drop_na()
colnames(dist500m.df) <- c("x","y","distland")

ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_raster(data=dist500m.df, aes(x=x, y=y, fill=distland), alpha=0.9) +
  scale_fill_gradient2() +
  geom_sf(data=study, fill=NA, color="red")
```
