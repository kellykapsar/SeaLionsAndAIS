...
Source: [GLOBAL OCEAN 1/4° PHYSICS ANALYSIS AND FORECAST UPDATED DAILY](https://resources.marine.copernicus.eu/?option=com_csw&view=details&product_id=GLOBAL_ANALYSISFORECAST_PHY_CPL_001_015)

```{r sea surface height}

ssh <- nc_open("../Data_Raw/MetO-GLO-PHY-CPL-dm-SSH_1627072681703.nc")

{
  sink('../Data_Raw/MetO-GLO-PHY-CPL-dm-SSH_1627072681703.txt')
  print(ssh)
  sink()
}

# Read lat lon and time for each observation
lon <- ncvar_get(ssh, "lon")
lat <- ncvar_get(ssh, "lat", verbose = F)
t <- ncvar_get(ssh, "time")

head(lon)

# Read in data from the ssh variable and verify the dimensions of the array
ssh.array <- ncvar_get(ssh, "zos") # 3dim array
dim(ssh.array)

# Identify fill value and replace with NA
fillvalue <- ncatt_get(ssh, "zos","_FillValue")
fillvalue
...
t2 <- as.POSIXct("2016-01-01 00:00")+t
...
# Save output file
# writeRaster(ssh_month, "../Data_Processed/SSH_Monthly.nc",
