...
# Calculate average hourly movement rate (km/hr)
  ...
  sealis <- sealis %>% 
    mutate(x = st_coordinates(sealis)[,1], y = st_coordinates(sealis)[,2]) %>% 
    group_by(DeployID) %>%
...
yearmon <- seq(as.Date("2018/11/1"), as.Date("2020/7/1"), "month")
...
#               varname="intensity", varunit="m",
#               longname="Shipping intensity (m travelled per cell) -- raster brick to netCDF",
...
# writeRaster(landmask.test,"../Data_Processed/Landmask_GEBCO.tif")
```
...
# using this link: https://gis.stackexchange.com/questions/244364/least-cost-path-barrier-r-gdistance
# and this one: file:///C:/Users/Kelly%20Kapsar/Downloads/v76i13%20(1).pdf
# as tutorials. Not sure if i'm doing it correctly 
library(gdistance)
test <- gdistance::transition(landmask, transitionFunction=mean, directions=8)
test2 <- geoCorrection(test, type="c")

origin <- rasterToPoints(dist500m)[,1:2]
goal <- dist500m
values(goal)[values(dist500m) <= -400 | values(dist500m) >= -500] = NA
goal <- !is.na(goal)
goal <- rasterToPoints(goal)[,1:2]

plot(raster(test2))
sPath1 <- shortestPath(test2, origin, goal, output="SpatialLines")
lines(sPath1)

test <- gdistance::transition(dist500m, transitionFunction=mean, directions=8)
#################################################################
...
# Calculation from Bornemann et al. 2021 
...
# Fun animation of the raster brick
# animate(eke_brick, pause=0.5, n=1)

# Calculate mean monthly eke rasters 

#get the date from the names of the layers and extract the month
indices <- format(as.Date(names(eke_brick), format = "X%Y.%m.%d"), format = "%Y%m")
indices <- as.numeric(indices)

#sum layers
eke_month <- stackApply(eke_brick, indices, fun = mean)
# animate(eke_month, pause=0.5, n=1)

# plot monthly average eke data for November, 2018
ekemonth.df <- as.data.frame(eke_month[[1]], xy=TRUE)  %>% drop_na()
colnames(ekemonth.df) <- c("x","y","eke")

ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_raster(data=ekemonth.df, aes(x=x, y=y, fill=eke), alpha=0.9) +
  scale_fill_gradient2() +
  geom_sf(data=study, fill=NA, color="red")
...
# Save output file
# writeRaster(eke_month, "../Data_Processed/EKE_Monthly_20181101-20200531.nc",
#               overwrite=TRUE, format="CDF",
...
# Save output file
# writeRaster(eke2_brick, "../Data_Processed/eke2.tif")

# Fun animation of the raster brick
# animate(eke2_brick, pause=0.5, n=1)

# Calculate mean monthly eke2 rasters 

#get the date from the names of the layers and extract the month
indices <- format(as.Date(names(eke2_brick), format = "X%Y.%m.%d"), format = "%Y%m")
indices <- as.numeric(indices)

#sum layers
eke2_month <- stackApply(eke2_brick, indices, fun = mean)
# animate(eke2_month, pause=0.5, n=1)

# plot monthly average ssh data for November, 2018
eke2month.df <- as.data.frame(eke2_month[[1]], xy=TRUE)  %>% drop_na()
colnames(eke2month.df) <- c("x","y","eke")

ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_raster(data=eke2month.df, aes(x=x, y=y, fill=eke), alpha=0.9) +
  scale_fill_gradient2() +
  geom_sf(data=study, fill=NA, color="red")
...
# Save output file
# writeRaster(eke2_month, "../Data_Processed/EKE_Monthly_20200601-20200731.nc",
#               overwrite=TRUE, format="CDF",
#               varname="EKE", varunit="m^2/s^2",
#               longname="Eddy Kinetic Energy -- raster brick to netCDF",
#               xname="lon",   yname="lat",zname="time",
...
test <-nc_open("../Data_Processed/WIND_Monthly.nc")
