...
ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_sf(data=study, fill=NA, color="red")+
  geom_sf(data=sealis, aes(color=inbounds))
...
sst_brick <- brick(sst.array, xmn=min(lat), xmx=max(lat), ymn=min(lon), ymx=max(lon), 
...
names(sst_brick) <- as.character(t)

# Save output file
# writeRaster(sst_brick, "../Data_Processed/SST_AOOS_cropped_4336.tif")

# ADD sea lion location data
# Compare SST at sea lion locations with tag-derived temperature
