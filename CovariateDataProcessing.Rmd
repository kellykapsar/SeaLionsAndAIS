...
# st_write(sealis, "../Data_Processed/SSL_Telemetry_Clean_20210727.shp")
...
```{r GEBCO landmask}
landmask.test <- bathy
landmask.test[landmask.test > 0] <- 1
landmask.test[landmask.test < 0 | landmask.test == 0] <- NA

landmask.testDf <- as.data.frame(landmask.test, xy=TRUE) %>% drop_na()
colnames(landmask.testDf) <- c("x","y","land")

ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_raster(data=landmask.testDf, aes(x=x, y=y, fill=land), alpha=0.9) +
  scale_fill_gradient2() +
  geom_sf(data=study, fill=NA, color="red")
