...
1 km grid of distance to land (in m).  

```{r dist2land}
# Calculate distance to nearest NA cell for all cells in landmask raster 
distland <- landmask.r
values(distland)[values(distland) == 1] = NA
distland <- raster::distance(distland)

distland.df <- as.data.frame(distland, xy=TRUE)
colnames(distland.df) <- c("x","y","distland")

ggplot() +
  geom_sf(data=basemap.crop, fill="gray", color="black", lwd=0.5) +
  geom_raster(data=distland.df, aes(x=x, y=y, fill=distland), alpha=0.9) +
  scale_fill_gradient2() +
  geom_sf(data=study, fill=NA, color="red")
...
Calculate distance to 200m depth isobath 
...
# Convert raster centroids from water cells to lat/long coordinates 
landmask.coords <- landmask.df %>% filter(water==1) %>% st_as_sf(coords=c("x","y"), crs=6334) %>% 
  st_transform(4326) %>% 
  st_coordinates() %>% 
  as.data.frame()
...
# along the coastline
system.time(dland <- dist2isobath(depth, landmask.coords$X[1:10], landmask.coords$Y[1:10], isobath = 0))
system.time(d500m <- dist2isobath(depth, landmask.coords$X, landmask.coords$Y, isobath = -500))

# Add back in land data 
system.time(test <- left_join(landmask.df, dland, by=c("x","y")))

# Plot the bathymetry
plot(depth, image = TRUE, lwd = 0.1, land = TRUE,
bpal = list(c(0, max(depth), "grey"), c(min(depth), 0, blues)))
# Make the coastline more visible
plot(depth, deep = 0, shallow = 0, step = 0, lwd = 0.6, add = TRUE)
# Add 10 random points from within study area 
pts <- sample(1:length(landmask.coords$X),10,replace=F)
points(landmask.coords$X[pts], landmask.coords$Y[pts], pch = 21, bg = "orange2", cex = 0.8)
# Add great circle lines
linesGC(d200m[, 2:3], d200m[, 4:5])
