...
ssl <- readRDS("../Data_Processed/Telemetry/UsedAndAvail_WeeklyKDE_20220314.rds") 

covar_names <- c("bathymetry", "dist_land", "dist_500m", "slope", "sst", "wind", "logship", "logfish", "prox_fish_km", "prox_ship_km", "prox_fish_km_new", "prox_ship_km_new")
covar_names_proper <- c("Bathymetry (m)", "Distance to land (m)", "Distance to shelf break (m)", "Slope (degrees)", "Avg. sea surface\ntemperature (C)", "Avg. wind speed (m/s)", "Log(Shipping traffic (km))", "Log(Fishing traffic (km))", "Euc. Distance to fishing (km)", "Euc. Distance to shipping (km)", "LCP Distance to fishing (km)", "LCP Distance to shipping (km)")
...
ssl %>% dplyr::select(bathymetry, dist_land, dist_500m, slope, wind, logfish, logship, sst, prox_ship_km, prox_fish_km, prox_ship_km_new, prox_fish_km_new)%>% 
  ggcorr(label=T)
...
# Remove old Euclidean distance calculations and calculate pairwise correlations
ssl_new <- ssl %>% 
  dplyr::select(-prox_ship_km, -prox_fish_km) %>% 
  rename(prox_ship_km = prox_ship_km_new, prox_fish_km = prox_fish_km_new)

# Double check to make sure it worked
length(which(ssl$prox_fish_km_new == ssl_new$prox_fish_km))
length(which(ssl$prox_ship_km_new == ssl_new$prox_ship_km))
...
corrlist <- lapply(1:11, function(x) ssl_new %>% filter(ind_id == x) %>% 
