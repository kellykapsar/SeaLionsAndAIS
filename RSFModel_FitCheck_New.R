...
covar_labels <- data.frame(
  Name = covar_names, 
  Label=c("Bathymetry", "Distance to land", "Distance to shelf break", 
          "Slope", "SST", 
          "Wind speed", "Shipping traffic", "Fishing Traffic", 
          "Distance to fishing", "Distance to shipping"))
...
pop.p <- ggplot(data = topmodbetas) +
  ...
  geom_pointrange(aes(ymin = CI_L, ymax = CI_H, x = Label, y = mean, color=ind_id),
                  size = 0.8, position = position_jitter(0.4)) + 
  scale_color_manual(values=getPalette(11), labels=M_labels) +
