...
usedbuff <- readRDS("../Data_Processed/Telemetry/watersealibuffs.rds")
...
imap(dplyr::select(ssl, -deploy_id, -used, -choice_id, -year, -month, -weekofyear), ~ {
...
ssl %>% dplyr::select(bathymetry, dist_land, dist_500m, slope, ssh, eke, wind, fish, ship, sst) %>% ggcorr(label=T)
