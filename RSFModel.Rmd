...
imap(dplyr::select(ssl, -DeployID, -Used, -choice_id, -year, -month, -weekofyear), ~ {
...
imap(dplyr::select(ssl, -DeployID, -Used, -choice_id, -year, -month, -weekofyear), ~ {
  ggplot(ssl, aes(x=DeployID, y=.x, fill=DeployID)) +
    geom_violin() +
    geom_boxplot(width = 0.1, alpha=0.2, color="grey") +
    ylab(.y) +
...
# Maybe use this to figure out how to apply across columns
# https://stackoverflow.com/questions/61617075/apply-ggplot2-across-columns

violin(data=ssl, x=ssl$Used, y=ssl$Bathymetry, fill=ssl$Used)
