...
ssl4 <- ssl4 %>% st_drop_geometry() %>% rename(dist_land = distland, dist_500m = dist500m,
                  ...
                  dplyr::select(weeklyhr_id, deploy_id, used, year, weekofyear, northing, easting, all_of(covar_names)) 


ssl4 <- st_as_sf(ssl4, coords = c("northing", "easting"), crs=prj, remove=FALSE)

# Save output
saveRDS(ssl4, "../Data_Processed/Telemetry/UsedAndAvail_WeeklyMCP_20211011.rds")
st_write(ssl4, "../Data_Processed/Telemetry/UsedAndAvail_WeeklyMCP_20211011.shp")

# See how many complete cases are left in available 
test <- ssl4 %>% st_drop_geometry() %>% filter(complete.cases(.))
temp <- test %>% group_by(weeklyhr_id, used) %>% summarize(n=n())

test <- data.frame(matrix(unlist(ssl4), nrow=nrow(ssl4)),stringsAsFactors=FALSE)
```
