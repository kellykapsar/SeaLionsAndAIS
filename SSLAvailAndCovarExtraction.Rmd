...
st_write(ssl_hr, "../Data_Processed/Telemetry/Homerange_KDE_weekly_20211029.shp")
...
# ID choice sets for discrete choice model 
# NOTE: I'M PRETTY SURE THIS IS AN INCORRECT WAY TO PARAMETERIZE THIS MODEL, BUT I'M DOING IT JUST TO SEE IF I CAN GET THE 
# EXAMPLE STAN CODE RUNNING BECAUSE THAT'S HOW IT'S ALREADY CODED... 
temp1 <- ssl_orig %>% dplyr::filter(used == 0) %>% group_by(weeklyhr_id) %>% mutate(choice_id = rep(1:(length(year)/5), each=5))
temp2 <- ssl_orig %>% dplyr::filter(used == 1) %>% group_by(weeklyhr_id) %>% mutate(choice_id = 1:length(year))

ssl <- rbind(temp1, temp2)

# Reorder columns to a more logical order and remove any rows with NA values for covariates
# Create a list of covariate names (NEW)
covar_names <- c("bathymetry", "dist_land", "dist_500m", "slope", "sst", "eke", "ssh", "wind", "logship", "logfish", "prox_fish_km", "prox_ship_km")

colorder <- c("deploy_id", "ind_id", "weeklyhr_id", "year", "choice_id", "used", "northing", "easting", all_of(covar_names))

ssl <- ssl[,colorder] %>% na.omit() %>% as.data.frame()

# Remove choice_ids with insufficient sample size due to NA values 
tooshort <- ssl %>% group_by(weeklyhr_id, choice_id) %>% summarize(n=n()) %>% filter(n < 6) %>% mutate(tooshort=1)
ssl <- left_join(ssl, tooshort, by=c("weeklyhr_id" = "weeklyhr_id", "choice_id" = "choice_id")) 
ssl <- ssl[which(is.na(ssl$tooshort)),]

# Recalculate choice sets now that short ones have been removed
temp1 <- ssl %>% dplyr::filter(used == 0) %>% group_by(weeklyhr_id) %>% mutate(choice_id = rep(1:(length(year)/5), each=5))
temp2 <- ssl %>% dplyr::filter(used == 1) %>% group_by(weeklyhr_id) %>% mutate(choice_id = 1:length(used))
...
ssl_nocoords <- ssl %>% select(-easting, -northing, -n, -tooshort, -geometry) %>%  as.data.frame()
saveRDS(ssl_nocoords, "../Data_Processed/Telemetry/UsedAndAvail_WeeklyKDE_20211029.rds")
write.csv(ssl_nocoords, "../Data_Processed/Telemetry/UsedAndAvail_WeeklyKDE_20211029.csv")

# Make it spatial again
ssl <- st_as_sf(ssl, coords = c("easting", "northing"), crs=prj, remove=FALSE)

# Save output as spatial object
st_write(ssl, "../Data_Processed/Telemetry/UsedAndAvail_WeeklyKDE_20211029.shp")
# browseURL("https://www.youtube.com/watch?v=K1b8AhIsSYQ")
