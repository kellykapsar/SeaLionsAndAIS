...
# files <- list.files("../Data_Processed/Telemetry/ssl5_season", pattern = "SSL2")
...
# }
# 
# saveRDS(ssl5, "../Data_Processed/Telemetry/ssl5_season/ssl5_season.rds")

ssl5 <- readRDS("../Data_Processed/Telemetry/ssl5_season/ssl5_season.rds")
...
saveRDS(ssl, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_ALLCases_20220615.rds")
write.csv(ssl, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_ALLCases_20220615.csv")

# Make it spatial again
ssl_sf <- st_as_sf(ssl, coords = c("easting", "northing"), crs=prj, remove=FALSE)

# Save output as spatial object
st_write(ssl_sf, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_ALLCases_20220615.shp")
...
tooshort <- ssl_narrow %>% group_by(seasonhr_id, choice_id) %>% summarize(n=n()) %>% filter(n < 6) %>% mutate(tooshort=1)
ssl_narrow <- left_join(ssl_narrow, tooshort, by=c("seasonhr_id" = "seasonhr_id", "choice_id" = "choice_id"))
...
saveRDS(ssl_narrow_nocoords, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_20220615.rds")
write.csv(ssl_narrow_nocoords, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_20220615.csv")

# Make it spatial again
ssl_narrow <- st_as_sf(ssl_narrow, coords = c("easting", "northing"), crs=prj, remove=FALSE)

# Save output as spatial object
st_write(ssl_narrow, "../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_20220615.shp")
