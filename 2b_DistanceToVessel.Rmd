...
#################################################
ssl6 <- readRDS("../Data_Processed/Telemetry/ssl6.rds")

# Turn used into a factor, log transform skewed vars, and drop geometry
ssl_new <- ssl6 %>% mutate(used = as.factor(used), 
                           logfish = log(fish+1), 
                           logship = log(ship+1)) %>% 
                    st_drop_geometry()

# Create an individual id
ssl <- ssl_new %>% group_by(deploy_id) %>% mutate(ind_id = cur_group_id()) %>% select(-prox_fish_km, -prox_ship_km) %>% rename(prox_ship_km = prox_ships_km_new, prox_fish_km = prox_fish_km_new)
