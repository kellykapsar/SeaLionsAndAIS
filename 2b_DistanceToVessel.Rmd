...
extractlanddist <- function(points, ships, fish, land, hrs){
  ships <- ships[which(ships$date == points$date[1] & ships$AIS_Typ != "Fishing"),]
  fish <- ships[which(fish$date == points$date[1] & fish$AIS_Typ == "Fishing"),]
  
  polyhr <- hrs[which(hrs$deploy_id == points$deploy_id[1] & 
                   hrs$year == points$year[1] &
                   hrs$week == points$weekofyear[1]),]
...
shipsin <- lines[st_intersects(polyhr, lines, sparse=FALSE),]


template <- raster(polyhr, res=250, crs=st_crs(polyhr)) # 100 m resolution
values(template) <- 0
...
points$prox_ship_km_new <- NA

cor.test(points$prox_ship_km, points$prox_ship_km_new, method="spearman")

points$prox_ship_km_new  <- lapply(points$geometry, function(x){distshp(x, distshipbuff)})
