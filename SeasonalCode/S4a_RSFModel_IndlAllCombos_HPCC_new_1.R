...
rs.data.path <- paste("../Data_Processed/UsedAndAvail_SeasonKDE_20220615.rds", sep = "")
# read in resource selection data and sort with used point at top of list
rs_data <- readRDS(rs.data.path) %>% group_by(seasonhr_id) %>% mutate(seasonhr_id = cur_group_id())
...
corrs <- read.csv(paste0("../Data_Processed/Corrs_season_", ind, ".csv"))
