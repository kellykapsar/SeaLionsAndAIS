...
ssl <- readRDS("../Data_Processed/Telemetry/UsedAndAvail_SeasonKDE_20220615.rds") 
...
# Calculate duration of tags in seasons 
tagduration <- used %>% group_by(deploy_id) %>% summarize(nseasons = length(unique(seasonhr_id)))

mean(tagduration$nseasons)
sd(tagduration$nseasons)
...
# Calculate average seasonal transmission rate
seasonaltransmissions <- used %>% group_by(deploy_id, seasonhr_id) %>% summarize(n = n()) %>% # Number of transmissions per season
                        # Average number of seasonal transmissions per individual
                        ungroup() %>% group_by(deploy_id) %>% summarize(avgseason = mean(n)) 

mean(seasonaltransmissions$avgseason)
sd(seasonaltransmissions$avgseason)
...
for(i in 1:11){
  ...
  ggsave(plot=t, filename =paste0("../Figures/Figures_Seasonal/PairCorrs_season_", dep_id, ".jpeg"), 
...
lapply(1:10, function(i){
  ...
  ggsave(filename=paste0("../Figures/Figures_Seasonal/UsedVsAvail_ByIndl_season_", covar_names[i], ".png"), width = 10, height = 7, units="in")
})
  ...
  ggplot(ssl, aes_string(x="used", y=covar_names[i], fill="used")) +
    ...
    scale_x_discrete(labels=c("0" = "Available", "1" = "Used")) +
      theme(axis.text.x = element_text(size=15), 
            ...
            axis.title=element_text(size=20), 
            legend.position="None") 
  ggsave(filename=paste0("../Figures/Figures_Seasonal/UsedVsAvail_season_", covar_names[i], ".png"), width = 10, height = 7, units="in")
})
...
lapply(1:10, function(i){
  ggplot(ssl, aes_string(x="deploy_id", y=covar_names[i], fill="deploy_id")) +
      ...
      theme(axis.text.x = element_text(angle = 55, size=15, hjust=1, vjust=1), 
            ...
            axis.title=element_text(size=20), 
            legend.position="None") 
  ggsave(filename=paste0("../Figures/Figures_Seasonal/UsedByIndl_season_", covar_names[i], ".png"), width = 10, height = 5, units="in")
})
...
lapply(1:11, function(x)write.csv(corrlist[[x]], paste0("../Data_Processed/Telemetry/Corrs_season_", x, ".csv")))
