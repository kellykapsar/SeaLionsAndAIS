...
covar_names_proper <- c("Bathymetry (m)", "Distance to land (m)", "Distance to shelf break (m)", "Slope (degrees)", "Avg. sea surface\ntemperature (C)", "Avg. wind speed (m/s)", "Log(Shipping traffic (km))", "Log(Fishing traffic (km))", "Proximity to fishing (km)", "Proximity to shipping (km)")
...
for(i in 1:11){
  print(i)
  t <- ssl %>% dplyr::filter(used == 1, ind_id == i) %>% dplyr::select(all_of(covar_names)) %>% ggpairs()
  ggsave(plot=t, filename =paste0("../Figures/PairCorrs_", i, ".jpeg"), 
