...
outpath <- paste("../Results/SwhrResults/output.Rout", sep = "")
...
rs.data.path <- paste("../Data_Processed/UsedAndAvail_Swhr_20220713.rds", sep = "")
...
ifelse(!dir.exists(paste0("../Results/SwhrResults/SSL_IndTEST", ind, "/")), 
       dir.create(paste0("../Results/SwhrResults/SSL_IndTEST", ind, "/")), FALSE)
...
corrs <- read.csv(paste0("../Data_Processed/Corrs_Swhr_", ind, ".csv"))
...
write.csv(master, paste0("../Results/SwhrResults/ModelVariableList_TEST", ind, ".csv"))

fitlst <- c()

# loop through the models and fit each one
fitlst <- foreach(j = 1:10,.packages = c("rstan", "loo")) %dopar% {
  ...
  save(fit1, file=paste0("../Results/SwhrResults/SSL_IndTEST", ind, "/Ind_", ind, "_Model_", jnum, ".rda"))
...
# save model fit
filename <- paste('SSL_IndlAllCombos_IndTEST', ind, '_results_', datestr,
                  '_new.rda', sep = '')
filepath <- paste("../Results/SwhrResults/", filename, sep = '')
