...
ifelse(!dir.exists(paste0("../Results/SwhrResults/SSL_Ind", ind, "/")), 
       dir.create(paste0("../Results/SwhrResults/SSL_Ind", ind, "/")), FALSE)
...
write.csv(master, paste0("../Results/SwhrResults/ModelVariableList_", ind, ".csv"))
...
fitlst <- foreach(j = 1:length(master$Var1),.packages = c("rstan", "loo")) %dopar% {
  ...
  save(fit1, file=paste0("../Results/SwhrResults/SSL_Ind", ind, "/Ind_", ind, "_Model_", jnum, ".rda"))
